...
export default function StockOut() {
    ...
    useEffect(() => {
        document.title = "Inventories - Stock Out";
        axios
            .get(
                `${appConfig.baseurlAPI}/stock-out?page=${currentPage}&per_page=${showing}&search=${searchTerm}&showing=${showing}`
            )
            .then((data) => {
    ...
    const handleSubmit = (event) => {
        ...
        if (!isEditing) {
            if (validateForm()) {
                axios
                    .post(`${appConfig.baseurlAPI}/stock-out`, formData, {
                        headers: {
            ...
            if (validateForm()) {
                axios
                    .put(
                        `${appConfig.baseurlAPI}/stock-out/${modalData.id}`,
                        formData,
                        {
                            headers: {
                                "Content-Type": "application/json",
    ...
    return (
        <Case>
            <div className="section-header px-4 tw-rounded-none tw-shadow-md tw-shadow-gray-200 lg:tw-rounded-lg">
                <h1 className="mb-1 tw-text-lg">Stock Out</h1>
            </div>

            <div className="section-body">
                <div className="card">
                    <div className="card-body px-0">
                        <h3>Table Stock Out</h3>
                        <SearchEntries
                            showing={showing}
