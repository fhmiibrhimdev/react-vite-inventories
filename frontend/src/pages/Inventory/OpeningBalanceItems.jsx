...
export default function OpeningBalanceItems() {
    ...
    useEffect(() => {
        document.title = "Inventories - Opening Balance Items";
        axios
            .get(
                `${appConfig.baseurlAPI}/opening-balance-items?page=${currentPage}&per_page=${showing}&search=${searchTerm}&showing=${showing}`
            )
            .then((data) => {
    ...
    const handleSubmit = (event) => {
        ...
        if (!isEditing) {
            if (validateForm()) {
                axios
                    .post(
                        `${appConfig.baseurlAPI}/opening-balance-items`,
                        formData,
                        {
                            headers: {
            ...
            if (validateForm()) {
                axios
                    .put(
                        `${appConfig.baseurlAPI}/opening-balance-items/${modalData.id}`,
                        formData,
                        {
                            headers: {
                                "Content-Type": "application/json",
    ...
    return (
        <Case>
            <div className="section-header px-4 tw-rounded-none tw-shadow-md tw-shadow-gray-200 lg:tw-rounded-lg">
                <h1 className="mb-1 tw-text-lg">Opening Balance Items</h1>
            </div>

            <div className="section-body">
                <div className="card">
                    <div className="card-body px-0">
                        <h3>Table Opening Balance Items</h3>
                        <SearchEntries
                            showing={showing}
