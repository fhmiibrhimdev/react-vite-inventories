...
export default function Category() {
    ...
    useEffect(() => {
        ...
        axios
            .get(
                `${appConfig.baseurlAPI}/category?page=${currentPage}&per_page=${showing}&search=${searchTerm}&showing=${showing}`
            )
            .then((data) => {
    ...
    const handleSubmit = (event) => {
        ...
        if (!isEditing) {
            if (validateForm()) {
                axios
                    .post(`${appConfig.baseurlAPI}/category`, formData, {
                        headers: {
            ...
            if (validateForm()) {
                axios
                    .put(
                        `${appConfig.baseurlAPI}/category/${modalData.id}`,
                        formData,
                        {
                            headers: {
                                "Content-Type": "application/json",
    ...
    const handleDelete = (id) => {
        axios
            .delete(`${appConfig.baseurlAPI}/category/${id}`)
            .then((data) => {
    ...
    return (
        <Case>
            <div className="section-header px-4 tw-rounded-none tw-shadow-md tw-shadow-gray-200 lg:tw-rounded-lg">
                <h1 className="mb-1 tw-text-lg">Category</h1>
            </div>
